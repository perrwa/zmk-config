...
- `dorny/paths-filter@v3` needs a real checkout for `push` events (it diffs before/after SHAs with local git) but not for `pull_request` events (it queries the GitHub API instead). `check-changes` in `build.yml` ran fine on PRs for months with no `actions/checkout` step; the first push-triggered run (adding the `zmk-v0.4` push trigger) failed with `fatal: not a git repository`. Gate the checkout step on `github.event_name == 'push'` rather than adding it unconditionally, so PR runs stay fast.
