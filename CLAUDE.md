...
`config/blecorne.keymap`, `blecorne.conf`, `blecorne_left.conf`, and `blecorne_right.conf` are symlinks to the corresponding `corne*` files — same bindings, same settings apply cleanly. This overrides `blecorne_left`'s own board `Kconfig.defconfig`, which (assuming standalone use) defaults it to `CONFIG_ZMK_SPLIT_ROLE_CENTRAL=y` and enables Studio/USB; the symlinked `corne_left.conf`/`corne.conf` force `n` for the dongle-central topology (also disabling Studio there, since on-device keymap edits would diverge from git). `config/blecorne.overlay` is a real file — BLE Corne's devicetree defines no `ext_power` driver node, but `corne.keymap`'s Numpad layer binds `&ext_power EP_OFF`; it's wired to GPIO 0.31, the expansion-header VCC switch pin documented in Boardsource's README (real behavior — cuts power to expansion-header peripherals, not the MCU/BLE — since `zmk,ext-power-generic` requires `control-gpios`, so a true no-op isn't possible). The driver node is a bare `EXT_POWER { ... }` — **no `ext_power:` label** — because ZMK's own `app/dts/behaviors/ext_power.dtsi` already puts that label globally on the `&ext_power` *behavior* node every keymap actually references; a second node with the same label is a devicetree error (`Label 'ext_power' appears on /behaviors/extpower and on /ext_power`). ZMK's own nice_nano/nrfmicro board overlays follow this same bare-name convention.
